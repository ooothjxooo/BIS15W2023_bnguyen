...
2. Every obervation has its own row.
3. Every own cell has its own value.
...
```
The reason we put parenthesis is to tell R that lakeid and annnumber is not an object.
...
```{r}
rename(mammals, genus = Genus, afr = AFR, )
```
