...
msleep %>% 
  ...
  summarize(mean_bwprimates = mean(bodywt),
            max_bwprimates= max(bodywt),
            min_bwprimates= min(bodywt),
            total=n())
...
```{r}
msleep %>% 
  summarise(slmax = max(sleep_total),
            slmin = min(sleep_total),
            slmean= mean(sleep_total),
            total=n())
```
...
msleep %>% 
  group_by(order) %>% 
  summarise(mean_bw=mean(brainwt))
```
