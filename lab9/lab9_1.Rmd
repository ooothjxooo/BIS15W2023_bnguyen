...
+ `discrete` quantitative data that only contains integers. 1 computer
+ `continuous` quantitative data that can take any numerical value 5.4 temperature
...
#when you choose a data for a plot, chose data that has mix of categorial and continuous data.
...
```{r}
ggplot(homerange, mapping = aes(x=log10.mass, y=log10.hra)) +geom_jitter() + geom_smooth()
```
...
```{r}
ggplot(homerange, mapping = aes(x=log10.hra,y=log10.preymass)) + geom_point(na.rm = T)+ geom_smooth(method = lm)
```

#502 rows were remove. This is another way to check if NA are presented
...
homerange %>% 
  ...
  geom_col()
```
#x variable has to be categorial
# if you fip them 
...
```{r}
ggplot(homerange_mammals, mapping=aes(x=trophic.guild)) + geom_bar()
```
